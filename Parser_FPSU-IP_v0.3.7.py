...
class Config():
    """Класс для описания конфигурации ФПСУ-IP и методов ее обработки"""
...
def menu(option):
    ...
    with open (EXEC_DIR + '\\fpsu_pars_opt{}_{}-{}.csv'.format(option, DATE.day, DATE.month), 'w') as w_file:
        if option in '1':
